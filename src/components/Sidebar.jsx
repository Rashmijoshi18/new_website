...
const ProfileAvatar = ({ size = "md", className = "" }) => {
	...
	return (
		<div
			className={`rounded bg-theme-sidebar-accent text-white flex items-center justify-center font-bold border-2 border-white border-opacity-30 shadow-lg ${sizeClasses[size]} ${className}`}
		>
			R
		</div>
	);
...
const DesktopSidebar = ({ expanded, onToggle }) => (
	...
	>
		...
		<button
			onClick={onToggle}
			className="absolute top-24 -right-3 w-6 h-6 bg-theme-primary hover:bg-primary-hover text-white rounded-full flex items-center justify-center shadow-lg transition-colors z-10"
			aria-label={expanded ? "Collapse sidebar" : "Expand sidebar"}
		>
			{expanded ? <ChevronLeft className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
		</button>
