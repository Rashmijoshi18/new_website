		...
		<>
			...
			<div className="lg:hidden fixed top-0 left-0 right-0 z-50 flex items-center justify-between bg-white border-b-2 border-[#308D46] px-4 py-3 shadow-md">
				<Link to="/" className="flex items-center gap-3">
					...
					<div className="flex flex-col">
						...
						<span className="text-xs text-[#308D46] font-semibold">rashmi3699</span>
					</div>
				</Link>
				<button onClick={() => setOpen(!open)} className="p-2 rounded hover:bg-gray-100 transition-colors">
					{open ? <X className="w-6 h-6 text-gray-700" /> : <Menu className="w-6 h-6 text-gray-700" />}
				</button>
			</div>

			{/* Mobile Drawer Backdrop */}
			{open && <div className="lg:hidden fixed inset-0 backdrop-blur-xs z-40" onClick={() => setOpen(false)} />}
			...
			>
				...
				<div className="p-4 border-t border-gray-200 bg-gray-50">
					<div className="text-xs text-gray-500 text-center">Made with ❤️ by Rashmi Joshi</div>
				</div>
			</div>
			...
			>
						...
						<div className="flex items-center gap-4 mb-3">
							...
							{expanded && (
								<div>
									...
									<div className="text-sm opacity-90">rashmi3699</div>
								</div>
							)}
						</div>
						{expanded && (
				...
				{/* Footer */}
				<div
					className={`p-4 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-green-50 ${
						!expanded ? "text-center" : ""
					}`}
				>
					{expanded ? (
						<div>
							<div className="text-xs font-semibold text-[#308D46] mb-2">Full Stack Developer</div>
							<div className="text-xs text-gray-600 leading-relaxed">Practice • Learn • Improve</div>
						</div>
					) : (
